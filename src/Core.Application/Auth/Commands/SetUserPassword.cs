...
{
    [SensitiveData]
    public record Command(Guid UserId, string Password) : IRequest;
    ...
    public class Handler(IRepository<User> users, IPasswordHash hasher, ILogger<Handler> log) : IRequestHandler<Command>
    {
        public async Task Handle(Command command, CancellationToken cancellationToken)
        {
            ...
            log.LogInformation("Setting password for user {Name} ({Email})", user.Name, user.Email);
            user.ForgotPassword();
            user.ResetPassword(user.ForgottenToken, command.Password, hasher);
        }
    }
}